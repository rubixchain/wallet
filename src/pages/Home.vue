      ...
      <section class="text-white bg-gray-900 body-font">
        <div class="container px-5 py-5 mx-auto flex items-center md:flex-row flex-col">
          <div class="flex flex-col md:pr-10 md:mb-0 mb-6 pr-0 w-full md:w-auto md:text-left text-center">
            <h2 class="text-xs text-indigo-400 tracking-widest font-medium title-font mb-1">{{dashboardData.nickname}}</h2>
            <h1 class="md:text-l text-xl font-medium title-font text-white">{{dashboardData.did}}</h1>
          </div>
          <div class="flex md:ml-auto md:mr-0 mx-auto items-center flex-shrink-0 space-x-4">
            <button class="bg-gray-800 inline-flex py-3 px-5 rounded-lg items-center  hover:bg-gray-700 hover:bg-opacity-50 focus:outline-none">
              <span class="ml-4 flex items-start flex-col leading-none">
                <span class="text-xs text-blue-700 text-gray-500 mb-1">Proof Credits</span>
            ...
            <button @click="$router.push({name:'Wallet'})" class="bg-gray-800 inline-flex py-3 px-5 rounded-lg items-center  hover:bg-gray-700 hover:bg-opacity-50 focus:outline-none">
              <span class="ml-4 flex items-start flex-col leading-none">
                <span class="text-xs text-blue-700 text-gray-500 mb-1">Balance</span>
            ...
            <button @click="$router.push({name:'Transactions'})" class="bg-gray-800 inline-flex py-3 px-5 rounded-lg items-center  hover:bg-gray-700 hover:bg-opacity-50 focus:outline-none">
              <span class="ml-4 flex items-start flex-col leading-none">
                <span class="text-xs text-blue-700 text-gray-500 mb-1">Transactions</span>
      ...
      <section class="text-white bg-gray-900 body-font">
        <div class="container px-0 py-10 mx-auto flex flex-wrap">
          <div class="flex flex-wrap w-full">
            ...
            <div class="lg:w-1/3 md:w-1/3 bg-gray-800 bg-opacity-50 rounded-lg p-2 flex flex-col md:ml-auto w-full mt-10 md:mt-0">
              ...
              <!-- <div class="relative mb-4">
                ...
                <label class="pr-2 leading-7 text-l text-white" for="Nickname"> Nickname</label>
                <input type="radio" id="DID" value="DID" v-model="picked">
                <label class="leading-7 text-l text-white" for="DID"> DID</label>
              </div> -->
              <div class="relative mb-4">
                <!-- <label for="full-name" class="leading-7 text-l text-white">{{picked}}</label> -->
                <label for="full-name" class="leading-7 text-l text-white">Receiver DID / Nickname</label>
                <textarea v-model="newTxn.receiver" spellcheck="false" type="text" id="full-name" name="full-name" class="w-full bg-white bg-gray-600 bg-opacity-20 focus:bg-transparent focus:ring-2 focus:ring-indigo-900 rounded border border-gray-600 focus:border-red-500 border-indigo-500 text-base outline-none text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"></textarea>
              </div>
              <div class="relative mb-4">
                <label for="email" class="leading-7 text-l text-white">Amount</label><br>
                <span class="pr-2 title-font font-medium">RBX</span>
                <input v-model="newTxn.tokenCount" type="email" id="email" name="email" class="w-1/3 bg-white bg-gray-600 bg-opacity-20 focus:bg-transparent focus:ring-2 focus:ring-indigo-900 rounded border border-gray-600 focus:border-red-500 border-indigo-500 text-base outline-none text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
              </div>
              <div class="relative mb-4">
                <label for="email" class="leading-7 text-l text-white">Comments</label>
              ...
              <button :disabled='dashboardData.balance==0' @click="initiateTransaction()" class="text-white border-0 py-2 px-8 focus:outline-none rounded text-lg" v-bind:class="{'bg-gray-300': dashboardData.balance==0, 'bg-indigo-500   hover:bg-indigo-600': dashboardData.balance!=0}">Initiate Transaction</button>
