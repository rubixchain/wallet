      ...
      <section class="text-gray-400 bg-gray-900 body-font">
        <div class="container px-5 py-10 mx-auto flex flex-wrap flex-col">
          <div class="flex mx-auto flex-wrap mb-20">
            <a v-on:click="toggleTabs(1)" class="sm:px-6 py-3 w-1/2 sm:w-auto justify-center sm:justify-start border-b-2 title-font font-medium inline-flex items-center leading-none border-gray-800 hover:text-white tracking-wider" v-bind:class="{'bg-gray-800 border-red-500 border-indigo-500 text-white rounded-t': openTab === 1, '': openTab !== 1}">
            ...
            <a v-on:click="toggleTabs(2)" class="sm:px-6 py-3 w-1/2 sm:w-auto justify-center sm:justify-start border-b-2 title-font font-medium inline-flex items-center leading-none border-gray-800 hover:text-white tracking-wider" v-bind:class="{'bg-gray-800 border-red-500 border-indigo-500 text-white rounded-t': openTab === 2, '': openTab !== 2}">
              <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-5 h-5 mr-3" viewBox="0 0 24 24">
                <path d="M22 12h-4l-3 9L9 3l-3 9H2"></path>
              </svg>Sent
            </a>
            <a v-on:click="toggleTabs(3)" class="sm:px-6 py-3 w-1/2 sm:w-auto justify-center sm:justify-start border-b-2 title-font font-medium inline-flex items-center leading-none border-gray-800 hover:text-white tracking-wider" v-bind:class="{'bg-gray-800 border-red-500 border-indigo-500 text-white rounded-t': openTab === 3, '': openTab !== 3}">
              <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-5 h-5 mr-3" viewBox="0 0 24 24">
                <path d="M22 12h-4l-3 9L9 3l-3 9H2"></path>
              </svg>
              Recieved
...
<section class="text-gray-400 bg-gray-900 body-font">
  <div class="container px-5 mx-auto flex flex-wrap">
    <div v-for="(t, index) in txns" :key="(t, index)" class="flex relative pb-8 sm:items-center md:w-2/3 mx-auto">
      ...
      <div class="flex-shrink-0 w-6 h-6 rounded-full mt-10 sm:mt-0 inline-flex items-center justify-center tbg-indigo-500  text-white relative z-10 title-font font-medium text-l">{{index+1}}</div>
      <div class="flex-grow md:pl-8 pl-6 flex sm:items-center items-start flex-col sm:flex-row">
        <div class="flex-grow sm:pl-3 mt-6 sm:mt-0">
          <h2 v-on:click="toggleModal(t.txn)" class="font-medium title-font text-white mb-1 text-xl">{{t.txn}}</h2>
          <span class="p-1 title-font font-medium text-white">{{t.nickname}}
                
                <button v-show="!t.edit" v-on:click="editContact(t)" class="pl-2 text-sm inline-flex items-center  bg-gray-800 border-0 py-0 px-2 focus:outline-none  hover:bg-gray-700 rounded mt-0 md:mt-0">
                  set nickname
                </button>
                <button v-show="t.edit" v-on:click="saveContact(t)" class="pl-2 text-sm inline-flex items-center  bg-gray-800 border-0 py-0 px-2 focus:outline-none  hover:bg-gray-700 rounded mt-0 md:mt-0">
                  save
                </button>
                <br>
              <input v-model="newNick" v-show="t.edit" v-if="true" type="nickname" id="nickname" name="nickname" class="w-1/2 bg-opacity-20 bg-transparent ring-2 ring-indigo-900 rounded border border-gray-600 border-red-500 border-indigo-500 text-base outline-none text-gray-900 text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
              </span>
          <div>
      ...
      <div class="relative w-auto my-6 mx-auto max-w-6xl">
        ...
        <div class="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
          ...
          <div class="flex items-center justify-end p-6 border-t border-solid border-blueGray-200 rounded-b">
            <button class="text-red-500 bg-transparent border border-solid border-red-500 hover: hover:text-white active:bg-red-600 font-bold uppercase text-sm px-6 py-3 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button" v-on:click="toggleModal()">
              Close
            </button>
          </div>
        </div>
