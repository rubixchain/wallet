  ...
  <div class="container px-5 py-24 mx-auto">
    ...
    <div class="flex lg:w-2/3 w-full sm:flex-row flex-col mx-auto px-8 sm:px-0 items-end sm:space-x-4 sm:space-y-0 space-y-4">
      <div class="relative sm:mb-0 flex-grow w-full">
        <label for="passphrase" class="leading-7 text-sm text-white">Passphrase</label>
        <input v-model="passphrase" type="text" id="passphrase" name="passphrase" class="w-full bg-gray-800 bg-opacity-40 rounded border border-gray-700 focus:border-red-500 border-indigo-500 focus:ring-2 focus:ring-indigo-900 focus:bg-transparent text-base outline-none text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
      </div>
      <div class="relative sm:mb-0 flex-grow w-full">
        <label for="email" class="leading-7 text-sm text-white">DID Image</label>
        ...
        <button @click="generate" class="text-white t bg-indigo-500  border-0 py-3 px-6 focus:outline-none hover:bg-red-300 hover:bg-indigo-600 rounded text-lg">Generate Image</button>
      </div>
      <button @click="create" class="text-white tbg-indigo-500  border-0 py-3 px-6 focus:outline-none hover:bg-red-300 hover:bg-indigo-600 rounded text-lg">Create</button>
    </div>
  </div>
</div>
    </div>
